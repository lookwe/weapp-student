  ...
  >
    <view>
      <view
        ...
        :class="iconClasses"
        :style="[isDefuCSS ? iconWrapStyle : coutomStyle]"
      >
        {{ indexToWORD(name) }}
      </view>
    </view>
...
import * as enums from '@/pages/answer/js/enumerate'
/**
 * checkbox  复选框
  ...
  computed: {
    ...
    elIconColor() {
      ...
      if (this.elDisabled) {
        ...
        return this.isChecked ? this.elInactiveColor : '#666'
      } else {
        return this.isChecked ? iconColor : '#666'
      }
    },
    iconClasses() {
      let classes = []
  ...
  methods: {
    init() {
      ...
      if (this.parentData.isDefuCSS === false) {
        if (this.isChecked || this.success == 1) {
          this.isDefuCSS = false
        }
      }
    },
    updateParentData() {
...
.u-checkbox {
  ...
  &__icon-wrap {
    ...
    &--disabled {
      background-color: $u-checkbox-icon-wrap-disabled-background-color;
    }

    &--disabled--checked {
      color: $u-checkbox-icon-wrap-disabled-checked-color;
    }
  }
